 ...
 */
package mining_private.ardougne;
 ...
 * @author t7emon
 */
@ScriptManifest(author = "T7emon", name = "Mining_Ardougne", version = 1.0, description = "Mining Ardougne", category = Category.MINING)
...
@Override
	...
	private State getState() {
        ...
        if (getInventory().count(mining_private.ardougne.Constants.ore) > new Random().nextInt(7 + 1) + 10 || getInventory().isFull()) {
        return State.DROP;
        } else {
      if (  mining_private.ardougne.Constants.ironOre.distance(getLocalPlayer().getTile()) < 2) {
     return State.MINE;
        }}
           ...
           GameObject ore2 = getGameObjects().getTopObjectOnTile(new Tile(2714, 3330, 0)); //7488 //7469
            if (ore1 != null && ore1.getID() == 11364) { 
              ...
              ore_count++;
                ...
                break;
               case DROP:
                   getInventory().dropAll(mining_private.ardougne.Constants.ore);
               break;
                case SLEEP:
